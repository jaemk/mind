...
| `mind sync [source] [--upgrade] [--dangerously-skip-install-hook-check] [--dangerously-skip-build-hook-check]` | refresh source clones (all, or only those matching the optional `[source]` selector: an exact name, an unambiguous trailing suffix, or a glob). `--upgrade` is deprecated sugar for `sync` followed by `upgrade` (the two hook-check flags are valid only with `--upgrade`) |
