...
| `mind unmeld <name> [--keep-items] [--yes] [--uninstall-hook <cmd>] [--dangerously-skip-hook-check]` | uninstall every item the source installed and drop the source (`--keep-items` skips the uninstall step). `<name>` may be a bare source name or, to address one specific instance, its full identity: `host/owner/repo@<prefix>` for an aliased instance, `host/owner/repo#<path>` for an item-link instance. The same identity forms select an instance for `upgrade` and `recall`, e.g. `mind unmeld github.com/acme/skills@jk` |
...
| `mind sync [source] [--upgrade] [--dangerously-skip-install-hook-check] [--dangerously-skip-build-hook-check]` | refresh source clones (all, or every source whose name matches the optional `[source]` selector: exactly, by trailing suffix, or by glob; a suffix shared by several sources matches all of them, since `sync` has no ambiguity check). `--upgrade` is deprecated sugar for `sync` followed by `upgrade`, scoped to the selected sources when `[source]` is given (the two hook-check flags are valid only with `--upgrade`) |
...
An [item link](#item-links-install-one-skill-by-url) instance is emitted as a
